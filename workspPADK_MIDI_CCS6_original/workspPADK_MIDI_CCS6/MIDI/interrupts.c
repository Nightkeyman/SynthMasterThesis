...
extern int whichwaveform = 0;
extern int overlap = 128;
		...
		//if(sem_dac == 0) {
			for(i = 0; i < FRAME_SIZE; i++) {
				...
				if (index < 1024-overlap) {
					...
					dmaxDacBuffer[!PP][0][0][i] = (1.0-wsp)*waveform[whichwaveform][index] + (wsp)*waveform[!whichwaveform][index];
					dmaxDacBuffer[!PP][1][0][i] = (1.0-wsp)*waveform[whichwaveform][index] + (wsp)*waveform[!whichwaveform][index];
					if (index >= 1024-1){
