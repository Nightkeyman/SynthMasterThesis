...
Inicjalizacja przetwornika cyfrowo analogowego polegała na ustawieniu między innymi identyfikatora zdarzeń dMAX oraz przerwania dedykowanego dla DAC. Priorytet przerwania został ustawiony na wysoki. Dodatkowo należało ustawić wartości rejestrów odpowiadających za prędkość próbkowania, wybór interfejsu przesyłającego dane z McASP do DAC oraz rejestr blokujący wydobywanie się sygnału na gniazdo zewnętrzne. Wszystkie wartości zostały przekazane za pomocą funkcji z bibliotek firmy Lyrtech o nazwie DAC\_Init().
...
Kolejnym etapem była inicjalizacja parametrów dotyczących ilości kanałów, stereo, rozmiaru ramki struktury DAC.

Częstotliwość próbkowania została ustawiona na 96kHz.
...
W literaturze można znaleźć informacje, iż widmo częstotliwościowe jest dużo bardziej czytelne i dokładne przy większej rozdzielczości częstotliwości. Minusem jej zwiększenia jest jednak spowolnienie obliczenia transformaty wynikowej i tym samym negatywny wpływ na czas pracy programu na procesorze.
