...
Proces identyfikacji przeprowadzono w środowisku symulacyjnym Matlab. Wybrano do niego te część nagrania dźwięku fletu, w której model instrumentu jest w stanie ustalonym. Na tym fragmencie sygnału przeprowadzono dyskretną transformację Fouriera za pomocą algorytmu FFT.
...
Każdy pik charakterystyki został przetworzony na dwa bieguny odbite lustrzanie względem osi realis na płaszczyźnie Z, według wzoru \ref{equ:flute_bieguny}. Każda z dolin została przetworzona na dwa zera odbite lustrzanie względem osi Z, według wzoru \ref{equ:flute_doliny}.

Model ARMA zsyntezowanego fletu utworzono na podstawie wyznaczonych zer i biegunów. W celu wygenerowania dźwięku z modelu, pobudzono go szumem białym, Następnie znormalizowano otrzymany sygnał, aby uniknąć jego przesterowań. Dźwięk wygenerowany z modelu przypomina brzmienie instrumentu z rodziny instrumentów dętych drewnianych. Wysokość dźwięku wydobytego tonu jest taka sama jak oryginalnego dźwięku (440Hz).
